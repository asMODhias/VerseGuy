...
async fn register_login_publish_verify() {
    ...
    // Publish plugin
    let manifest: Value = must(serde_json::from_str(r#"{ "manifest": { "id": "org.e2e.test", "name": "E2E Test Plugin", "version": "0.0.1", "author": "Tester", "description": "E2E", "published_at": null } }"#));
    ...
    let manifest: master_server::plugins::PluginManifest = must(serde_json::from_value(manifest_json.clone()));
