...
async fn verify_and_revoke_flow() {
    ...
    let pub_b64 = general_purpose::STANDARD.encode(must_opt(state.keypair.as_ref(), "missing keypair").public.to_bytes());
    ...
    let body = format!(r#"{{"manifest":{},"public_key_b64":"{}"}}"#, manifest_ser, pub_b64);
    ...
    assert!(must_opt(v.get("valid").and_then(|b| b.as_bool()), "missing valid"));

    // Revoke via admin endpoint
    std::env::set_var("MASTER_ADMIN_TOKEN", "admintoken");
    let revoke_body = r#"{"id":"org.test.signhttp","version":"0.1.0","reason":"compromised"}"#.to_string();
    ...
    assert!(!must_opt(v3.get("revocations").and_then(|r| r.as_array()), "missing revocations").is_empty());
}
