...
async fn create_and_list_orgs() {
    ...
    assert_eq!(must_opt(created.get("name").and_then(|n| n.as_str()), "missing name"), "TestOrg");

    // GET by id
    let id = must_opt(created.get("id").and_then(|v| v.as_str()), "missing id");
    let uri = format!("/v1/orgs/{}", id);
    let req3 = must(Request::builder().method("GET").uri(uri).body(Body::empty()));
    ...
    assert_eq!(must_opt(fetched.get("id").and_then(|v| v.as_str()), "missing id 2"), id);

    // List again should have one entry
    let req4 = must(Request::builder().method("GET").uri("/v1/orgs").body(Body::empty()));
    ...
    assert_eq!(must_opt(v4.get("orgs").and_then(|r| r.as_array()), "missing orgs 2").len(), 1);
}
