use anyhow::Result;
use plugins_adapter_scunpacked::{import_from_file, import_from_p4k, import_from_url, extract_p4k_to_dir, import_from_github};
use std::env;
...
fn main() -> Result<()> {
    ...
    let mut src_repo: Option<String> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--p4k-extract" => {
                ...
                let p = args[i].clone();
                // optional out dir
                let mut outdir = PathBuf::from("data/scunpacked/extracted");
                if i + 2 < args.len() && args[i + 1] == "--out-dir" {
                    i += 2;
                    outdir = PathBuf::from(&args[i]);
                }
                println!("Extracting p4k {} to {}", p, outdir.display());
                let n = extract_p4k_to_dir(&p, &outdir)?;
                println!("Extracted {} files to {}", n, outdir.display());
                return Ok(());
            }
            "--repo" => {
                i += 1;
                if i >= args.len() { usage_and_exit(); }
                src_repo = Some(args[i].clone());
            }
            _ => {
                usage_and_exit();
            }
    ...
    if src_file.is_none() && src_url.is_none() && src_repo.is_none() && src_p4k.is_none() {
        usage_and_exit();
    }
    ...
    } else if let Some(repo) = src_repo {
        println!("Importing from remote repo: {}", repo);
        import_from_github(&repo, &storage)?
