...
fn test_create_organization() {
    ...
    let org = match svc.create_organization("Test Org".into(), "TST".into(), "desc".into(), "owner1".into()) {
        Ok(o) => o,
...
fn test_list_members() {
    ...
    match svc.add_member(m1) { Ok(_) => {}, Err(e) => panic!("add_member failed: {}", e) }
    match svc.add_member(m2) { Ok(_) => {}, Err(e) => panic!("add_member failed: {}", e) }

    let list = match svc.list_members("org1") { Ok(l) => l, Err(e) => panic!("list_members failed: {}", e) };
...
fn test_permissions() {
    ...
    match storage.put(keys::rank("org1", &rank.id), &rank) { Ok(_) => {}, Err(e) => panic!("storage.put failed: {}", e) };
    ...
    match svc.add_member(member) { Ok(_) => {}, Err(e) => panic!("add_member failed: {}", e) };

    let has_perm = match svc.has_permission("u1", Permission::ManageMembers) { Ok(h) => h, Err(e) => panic!("has_permission failed: {}", e) };
    assert!(has_perm);
}