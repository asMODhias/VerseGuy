...
impl ApplicationService {
    ...
    pub fn create_organization(&self, dto: CreateOrganizationDto, _user: String) -> Result<Organization, ()> {
        if dto.tag.len() > 5 || dto.tag.chars().any(|c| !c.is_ascii_uppercase()) {
            return Err(());
        }
        let id = Uuid::new_v4().to_string();
        let org = Organization { id: id.clone(), name: dto.name, tag: dto.tag, description: dto.description, member_count: 1, treasury_balance: 0 };
    ...
    pub fn create_fleet(&self, dto: CreateFleetDto, _user: String) -> Result<Fleet, ()> {
        let id = Uuid::new_v4().to_string();
        let fleet = Fleet { id: id.clone(), organization_id: dto.organization_id, name: dto.name, ship_count: 0, total_crew: 0 };
        self.fleets.lock().unwrap().insert(id.clone(), fleet.clone());
        Ok(fleet)
    }
    ...
    pub fn create_operation(&self, dto: CreateOperationDto, _user: String) -> Result<Operation, ()> {
        let id = Uuid::new_v4().to_string();
        let op = Operation { id: id.clone(), organization_id: dto.organization_id, name: dto.name, status: "draft".to_string(), participant_count: 0 };
...
// DTOs
#[derive(Clone)]
pub struct CreateOrganizationDto { pub name: String, pub tag: String, pub description: String }
#[derive(Clone)]
pub struct AddMemberDto { pub organization_id: String, pub user_id: String }
#[derive(Clone)]
pub struct TreasuryOperationDto { pub organization_id: String, pub amount: i64, pub reason: Option<String> }
#[derive(Clone)]
pub struct CreateFleetDto { pub organization_id: String, pub name: String, pub description: String }
#[derive(Clone)]
pub struct AddShipDto { pub fleet_id: String, pub manufacturer: String, pub name: String, pub variant: Option<String>, pub role: String, pub owner_id: String, pub crew_size: i64, pub cargo_capacity: i64 }
#[derive(Clone)]
pub struct CreateOperationDto { pub organization_id: String, pub name: String, pub description: String, pub operation_type: String, pub scheduled_start: DateTime<Utc>, pub scheduled_end: DateTime<Utc> }
#[derive(Clone)]
pub struct AddParticipantDto { pub operation_id: String, pub user_id: String, pub role: String }

impl Default for ApplicationService { fn default() -> Self { Self::new() }}
