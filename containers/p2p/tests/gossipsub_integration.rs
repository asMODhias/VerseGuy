#![allow(clippy::disallowed_methods, clippy::collapsible_if, clippy::single_match, clippy::manual_unwrap_or_default, clippy::let_unit_value, clippy::collapsible_match, clippy::needless_bool)]
...
async fn gossipsub_publish_subscribe() {
    ...
    let cfg = verseguy_test_utils::must(libp2p::gossipsub::ConfigBuilder::default()
        .validation_mode(libp2p::gossipsub::ValidationMode::Permissive)
        .build());

    // Sign outgoing messages using the transport keypairs
    let gsub_a: libp2p_gossipsub::Behaviour<libp2p_gossipsub::IdentityTransform, libp2p_gossipsub::AllowAllSubscriptionFilter> = verseguy_test_utils::must(libp2p_gossipsub::Behaviour::new(libp2p_gossipsub::MessageAuthenticity::Signed(key_a.clone()), cfg.clone()));
    let gsub_b: libp2p_gossipsub::Behaviour<libp2p_gossipsub::IdentityTransform, libp2p_gossipsub::AllowAllSubscriptionFilter> = verseguy_test_utils::must(libp2p_gossipsub::Behaviour::new(libp2p_gossipsub::MessageAuthenticity::Signed(key_b.clone()), cfg.clone()));
    ...
    let transport_c = tcp_c
        .upgrade(libp2p::core::upgrade::Version::V1)
        .authenticate(verseguy_test_utils::must(libp2p::noise::Config::new(&key_c)))
        .multiplex(libp2p::yamux::Config::default())
        .boxed();

    let gsub_c: libp2p_gossipsub::Behaviour<libp2p_gossipsub::IdentityTransform, libp2p_gossipsub::AllowAllSubscriptionFilter> = verseguy_test_utils::must(libp2p_gossipsub::Behaviour::new(libp2p_gossipsub::MessageAuthenticity::Signed(key_c.clone()), cfg.clone()));
    ...
    let connected = {
        ...
        while std::time::Instant::now() < deadline {
            ...
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(200), futures::StreamExt::next(&mut swarm_a)).await {
                if let Some(ev) = ev_opt {
                    eprintln!("s1 event: {:?}", ev);
                    if let SwarmEvent::ConnectionEstablished { peer_id, .. } = ev {
                        if peer_id == peer_b || peer_id == peer_c { s1_ok = true; }
                    }
                    if let SwarmEvent::ConnectionClosed { .. } = ev { s1_ok = false; }
                }
            }
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(200), futures::StreamExt::next(&mut swarm_b)).await {
                if let Some(ev) = ev_opt {
                    eprintln!("s2 event: {:?}", ev);
                    if let SwarmEvent::ConnectionEstablished { peer_id, .. } = ev {
                        if peer_id == peer_a || peer_id == peer_c { s2_ok = true; }
                    }
                    if let SwarmEvent::ConnectionClosed { .. } = ev { s2_ok = false; }
                }
            }
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(200), futures::StreamExt::next(&mut swarm_c)).await {
                if let Some(ev) = ev_opt {
                    eprintln!("s3 event: {:?}", ev);
                    if let SwarmEvent::ConnectionEstablished { peer_id, .. } = ev {
                        if peer_id == peer_a || peer_id == peer_b { s3_ok = true; }
                    }
                    if let SwarmEvent::ConnectionClosed { .. } = ev { s3_ok = false; }
                }
            }
    ...
    while start.elapsed() < Duration::from_millis(4000) {
        if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(50), futures::StreamExt::next(&mut swarm_a)).await {
            if let Some(ev) = ev_opt {
                ...
                if let SwarmEvent::Behaviour(libp2p_gossipsub::Event::Subscribed { topic, .. }) = &ev {
                    if format!("{:?}", topic).contains("verseguy/global") { saw_subscription = true; }
                }
            }
        }
        ...
        while start_sub.elapsed() < Duration::from_secs(6) {
            ...
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(100), futures::StreamExt::next(&mut swarm_a)).await {
                if let Some(ev) = ev_opt {
                    ...
                    if let SwarmEvent::Behaviour(libp2p_gossipsub::Event::Subscribed { topic, .. }) = &ev {
                        if format!("{:?}", topic).contains("verseguy/global") { saw_subscription = true; break; }
                    }
                }
            }
            ...
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(100), futures::StreamExt::next(&mut swarm_c)).await {
                if let Some(ev) = ev_opt {
    ...
    for _ in 0..40 {
        match swarm_a.behaviour_mut().publish(topic.clone(), b"hello".to_vec()) {
            ...
            Err(e) => {
                ...
                if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(100), futures::StreamExt::next(&mut swarm_a)).await {
                    if let Some(ev) = ev_opt { eprintln!("s1 event during publish retry: {:?}, retrying", ev); }
                }
                if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(100), futures::StreamExt::next(&mut swarm_b)).await {
                    if let Some(ev) = ev_opt { eprintln!("s2 event during publish retry: {:?}, retrying", ev); }
                }
        ...
        if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(100), futures::StreamExt::next(&mut swarm_a)).await {
            if let Some(ev) = ev_opt {
                ...
                if let SwarmEvent::Behaviour(libp2p_gossipsub::Event::Message { message, .. }) = &ev {
                    if message.data == b"hello".to_vec() { saw_message = true; break; }
                }
            }
        }
        if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(100), futures::StreamExt::next(&mut swarm_b)).await {
    ...
    let got = tokio::time::timeout(Duration::from_secs(8), async {
        ...
        while std::time::Instant::now() < deadline {
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(200), futures::StreamExt::next(&mut swarm_b)).await {
                if let Some(ev) = ev_opt {
                    if let SwarmEvent::Behaviour(libp2p_gossipsub::Event::Message { message, .. }) = &ev {
                        ...
                        if message.data == b"hello".to_vec() { return true; }
                    }
                }
            }
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(200), futures::StreamExt::next(&mut swarm_c)).await {
            ...
            if let Ok(ev_opt) = tokio::time::timeout(Duration::from_millis(50), futures::StreamExt::next(&mut swarm_a)).await {
                if let Some(ev) = ev_opt { eprintln!("s1 pump while waiting for message: {:?}", ev); }
            }
        }
        false
