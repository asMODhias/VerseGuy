...
fn generate_certificate_and_parse_extension() {
    ...
    let (cert_der, _key_der) = tls::certificate::generate(&keypair).expect("generate");

    // Parse the certificate and ensure SPKI and signature exist
    let p2p_cert = tls::certificate::parse(&cert_der).expect("parse");
    ...
    let spki = p2p_cert.spki.unwrap();
    let sig = p2p_cert.signature.unwrap();
    ...
    let (_, spki_struct) = x509_parser::x509::SubjectPublicKeyInfo::from_der(&spki).expect("spki parse");
